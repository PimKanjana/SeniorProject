...
while 1:
    print("Press any key to continue D#1! (or press ESC to quit!)")
    if getch() == chr(0x1b):
        break

    # Write moving speed #1  
    dxl_comm_result, dxl_error = packetHandler.write2ByteTxRx(portHandler, DXL1_ID, ADDR_MOVING_SPEED, dxl1_speed )
    ...
    if dxl_comm_result != COMM_SUCCESS:
        ...
        print("%s" % packetHandler.getRxPacketError(dxl_error))
    print("[ID:%03d] PresPos:%03d" % (DXL1_ID, dxl1_present_position))
	...
	# Write moving speed #2  
    dxl_comm_result, dxl_error = packetHandler.write2ByteTxRx(portHandler, DXL2_ID, ADDR_MOVING_SPEED, dxl2_speed )
