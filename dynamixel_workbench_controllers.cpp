...
{
	ROS_ERROR("%s", log);
          ROS_ERROR(" write value[%d] on items[%s] to Dynamixel[Name : %s, ID : %d]", info.second.value, info.second.item_name.c_str(), dxl.first.c_str(), dxl.second);
}
        }
      }
    }
...
bool DynamixelController::initSDKHandlers(void)
{
  ...
  {  
    ...
    result = dxl_wb_->addSyncReadHandler(126,
                                          10,
                                          &log);
...
bool DynamixelController::getPresentPosition(std::vector<std::string> dxl_name)
{
  bool result = false;
  const char* log = NULL;
  int32_t get_current1[dynamixel_.size()];
  int32_t get_velocity1[3];
  int32_t get_position1[3];
  int32_t get_current2[2];
  int32_t get_velocity2[2];
  int32_t get_position2[2];

 

  uint8_t id_array[3] = {1,2,3};
  uint8_t id_array1[2] = {4,5};
  uint8_t id_cnt = 0;
  uint8_t id_cnt1 = 0;
...
result = dxl_wb_->syncRead(SYNC_READ_HANDLER_FOR_PRESENT_POSITION_VELOCITY_CURRENT,
	 ...
	 if (result == false)
      {
        ROS_ERROR("EEEE%s", log);
      }

     	   result = dxl_wb_->syncRead(SYNC_XL,
                                  id_array1,
                                  2,
                                  &log);
	 if (result == false)
 ...
 WayPoint wp;
  if (dxl_wb_->getProtocolVersion() == 2.0f)
  {
      result = dxl_wb_->getSyncReadData(SYNC_READ_HANDLER_FOR_PRESENT_POSITION_VELOCITY_CURRENT,
                                                    ...
                                                    get_position1,
                                                    &log);
result = dxl_wb_->getSyncReadData(SYNC_XL,
                                                    id_array1,
                                                    2,
                                                    control_items_["Present_Position"]->address,
                                                    control_items_["Present_Position"]->data_length,
    ...
    {
 int32_t get_position[dynamixel_.size()] = {get_position1[0],get_position1[1],get_position1[2],get_position2[0],get_position2[1]};
...
result = dxl_wb_->getSyncReadData(SYNC_XL,
      ...
      if (result == false)
      {
        ROS_ERROR("pp%s", log);
		 ...
		 int32_t get_current[dynamixel_.size()] = {get_current1[0],get_current1[1],get_current1[2],get_current2[0],get_current2[1]};
	 int32_t get_velocity[dynamixel_.size()] = {get_velocity1[0],get_velocity1[1],get_velocity1[2],get_velocity2[0],get_velocity2[1]};
	 int32_t get_position[dynamixel_.size()] = {get_position1[0],get_position1[1],get_position1[2],get_position2[0],get_position2[1]};
      for(uint8_t index = 0; index < id_cnt; index++)
      {
                                                      ...
                                                      dynamixel_workbench_msgs::DynamixelCommand::Response &res)
{
 ...
 // ROS_INFO("%d id, %s , %d ",id,item_name.c_str(),value);
	//if(item_name == "Goal_Position")
	//{
	//	value = dxl_wb_->convertRadian2Value(id,req.value);
	//}
